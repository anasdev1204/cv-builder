...
export const validateEntryContent = (value: string | SectionEntry[] | string[], t: TFunction) => {
    if (Array.isArray(value) && value.length > 0 && isSectionEntry(value[0])) {
        ...
        for (const entry of se) {
            ...
            if (entry.subtitle.trim().length < 2) return t("cv.editor.entryInput.subtitleValidationError");
            if (entry.end_date && entry.start_date && new Date(entry.end_date) < new Date(entry.start_date)) {
