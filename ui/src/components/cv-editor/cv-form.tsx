...
export default function CVForm({ t, cv, version, setVersion, addCVVersion, saveCV }: CVFormProps) {
  ...
  const form = useForm<CVRaw>({
    ...
    validate: {
      user_data: {
        name: (value) =>
          value.trim().length < 2 ? t('cv.editor.nameInput.validationError') : null,
        email: (value) =>
          /^\S+@\S+\.\S+$/.test(value) ? null : t('cv.editor.emailInput.validationError'),
        phone_number: (value) =>
        ...
        address: {
          country: (value) =>
            value.trim().length < 2 ? t('cv.editor.countryInput.validationError') : null,
          city: (value) =>
            value.trim().length < 2 ? t('cv.editor.cityInput.validationError') : null,
        },
      },
      sections: {
        [version || '']: {
          summary: {
            title: (value) =>
              value.trim().length < 2 ? t('cv.editor.summaryInput.titleValidationError') : null,
            content: (value) =>
          ...
          experience: {
            title: (value) =>
              value.trim().length < 2 ? t('cv.editor.experienceInput.titleValidationError') : null,
            content: (value) => validateEntryContent(value, t),
          },
          education: {
            title: (value) =>
              value.trim().length < 2 ? t('cv.editor.educationInput.titleValidationError') : null,
            content: (value) => validateEntryContent(value, t),
          },
          languages: {
            title: (value) =>
              value.trim().length < 2 ? t('cv.editor.languagesInput.titleValidationError') : null,
            content: (value) =>
              !Array.isArray(value) ||
          ...
          skills: {
            title: (value) =>
              value.trim().length < 2 ? t('cv.editor.skillsInput.titleValidationError') : null,
            content: (value) =>
              !Array.isArray(value) ||
              value.some((item) => typeof item !== 'string' || item.trim().length === 0)
                ? t('cv.editor.listInput.contentValidationError')
        ...
        <Box>
          ...
          <Flex gap="sm" mt="md" align="center">
            <TextInput
              ...
              style={{ flex: 1 }}
            />
            <Button onClick={handleAddCustomSection}>{t('cv.editor.form.addCustomSection')}</Button>
          </Flex>
