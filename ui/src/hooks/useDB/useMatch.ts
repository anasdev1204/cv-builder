...
export function useMatchedCV() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const matches = await db.matchedCVs.orderBy('createdAt').reverse().toArray();

      setData(matches);
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Failed to load matched CVs.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const save = useCallback(async (value: StoredMatchedCV) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to save matched CV.');
      throw error;
    } finally {
      setSaving(false);
    }
  ...
  const remove = useCallback(async (id: string) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to delete matched CV.');
      throw error;
    }
  }, []);
